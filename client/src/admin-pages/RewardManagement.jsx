  ...
  const fetchRewards = async () => {
    try {
      ...
      const rewardsWithImageUrls = response.data.map(reward => ({
        ...reward,
        id: reward._id,
        image: reward.image ? `http://capstone-ecocollect-production-da52.up.railway.app/${reward.image.path}` : null
      }));
  ...
  const handleRemove = async (id) => {
    ...
    try {
      if (!id.startsWith('new-')) {
        await axios.delete(`http://capstone-ecocollect-production-da52.up.railway.app/api/ecocollect/rewards/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (current.id.startsWith('new-')) {
        response = await axios.post("http://capstone-ecocollect-production-da52.up.railway.app/api/ecocollect/rewards", formData, {
          headers: { "Content-Type": "multipart/form-data" }
        });
        toast.success("Reward added successfully");
      } else {
        response = await axios.put(`http://capstone-ecocollect-production-da52.up.railway.app/api/ecocollect/rewards/${current.id}`, formData, {
          headers: { "Content-Type": "multipart/form-data" }
        });
